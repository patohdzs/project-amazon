...
data {
  ...
  real<lower=0> pa;
  real<lower=0> pf;
  vector[S] gamma_prior_mean;
  vector[S] gamma_prior_std;
  vector[S] theta_prior_mean;
  vector[S] theta_prior_std;
}
parameters {
  vector<lower=0>[S] gamma; // gamma
  vector<lower=0>[S] theta; // theta
}
model {
  // Priors
  gamma ~ lognormal(gamma_prior_mean, gamma_prior_std);
  theta ~ lognormal(theta_prior_mean, theta_prior_std);
