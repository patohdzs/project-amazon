...
data {
  ...
  matrix[S,S] gamma_prior_vcov;
  vector[S] theta_prior_mean;
  matrix[S,S] theta_prior_vcov;
}
parameters {
...
model {
  // Priors
  gamma ~ multi_normal(gamma_prior_mean, gamma_prior_vcov);
  theta ~ multi_normal(theta_prior_mean, theta_prior_vcov);
