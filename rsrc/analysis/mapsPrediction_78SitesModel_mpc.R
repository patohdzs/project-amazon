...
prediction.78SitesModel <- purrr::map_df(.x = aux.prices,
                                         .f = ~readr::read_delim(here::here("data/prediction/78-mpc",
                                                                            glue::glue("p_e_{.x}/amazon_data_z.dat")), skip = 1,
...
for (price in aux.prices) {
  ...
  # SAVE MAP Z0, Z50, GAMMA, THETA
  ggpubr::ggarrange(mapList[[mapIndex]]) %>%
    ggpubr::ggexport(filename = here::here(glue::glue("results/prediction/78-MPC/map_z30_78Sites_pe{price}.png")),
                     width = 2400, height = 1500)
...
ggpubr::ggarrange(z_2017_78Sites, 
                  ...
                  ncol = 4, nrow = 1) %>%
  ggpubr::ggexport(filename = here::here(glue::glue("results/prediction/78-MPC/map_z0z30GammaTheta_78Sites_allPrices_mpc.png")),
                   width = 2700, height = 1500)
...
for (y in seq_along(aux.years)) {
  ...
  ggplot2::ggplot(data = prediction.78SitesModel %>%
    ...
    ggplot2::theme(panel.grid.major = element_line(colour = "white"),
                   ...
                   legend.text = element_text(size = 28, face = "bold"),
                   plot.margin=unit(c(t=-0.5,r=-1,b=-0,l=-1), "cm"))
  ggplot2::ggsave(here::here(glue::glue("results/prediction/78-MPC/map_z{aux.years[y]}_78Sites_pe21.9.png")), width = 12, height = 9, dpi = 300)
