...
# z_2017_1043Sites
z_2017_1043Sites <-
  ggplot2::ggplot(data = prediction.1043SitesModel %>% dplyr::filter(time == 0, p_e == 6.5) %>%
                    dplyr::mutate(z_2017_1043Sites = cut(round(z_2017_1043Sites, 1), breaks = c(0,0.5,20,40,60,80,105), include.lowest = T, dig.lab = 3,
...
theta_1043Sites <-
  ggplot2::ggplot(data = prediction.1043SitesModel %>% dplyr::filter(time == 0, p_e == 6.5) %>%
                    dplyr::mutate(rank_theta_1043Sites = ggplot2::cut_interval(round(rank_theta_1043Sites),  n = 5, dig.lab = 3))) +
...
for (price in aux.prices) {
  ...
  mapList[[mapIndex]] <-
    ...
    ggpubr::ggarrange(mapList[[mapIndex]]) %>%
      ggpubr::ggexport(filename = here::here(glue::glue("results/prediction/1043SitesModel/map_z50_1043Sites_pe{price}.png")),
...
ggpubr::ggarrange(gamma_1043Sites, theta_1043Sites, z_2017_1043Sites, "",
                  ...
                  ncol = 4, nrow = 2) %>%
  ggpubr::ggexport(filename = here::here(glue::glue("results/prediction/1043SitesModel/map_z0z50GammaTheta_1043Sites_allPrices.png")),
...
for (p in seq_along(aux.prices)) {
  ...
  ggpubr::ggarrange(z_2017_1043Sites, aux.mapList[[2]], aux.mapList[[3]],
                    ...
                    ncol = 3, nrow = 2) %>%
    ggpubr::ggexport(filename = here::here(glue::glue("results/prediction/1043SitesModel/map_zDecades_1043Sites_pe{aux.prices[p]}.png")),
                     width = 2700, height = 1500)
...
for (y in seq_along(aux.years)) {
  ...
  # z50 map (vary by model)
    ...
    ggplot2::theme(panel.grid.major = element_line(colour = "white"),
                   ...
                   legend.text = element_text(size = 28, face = "bold"),
                   plot.margin=unit(c(t=-0.5,r=-1,b=-0,l=-1), "cm"))
  ggplot2::ggsave(here::here(glue::glue("results/prediction/1043SitesModel/map_z{aux.years[y]}_1043Sites_pe20.76.png")), width = 12, height = 9, dpi = 300)
