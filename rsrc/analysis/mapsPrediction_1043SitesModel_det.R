...
for (price in aux.prices) {
  ...
  ggpubr::ggarrange(mapList[[mapIndex]]) %>%
    ggpubr::ggexport(
      filename = here::here(glue::glue("plots/prediction/1043-det/map_z50_1043Sites_pe{price}.png")),
...
ggpubr::ggarrange(gamma_1043Sites, theta_1043Sites, z_2017_1043Sites, "",
  ...
  ggpubr::ggexport(
    filename = here::here(glue::glue("plots/prediction/1043-det/map_z0z50GammaTheta_1043Sites_allPrices_det.png")),
...
for (price in aux.prices) {
  ...
  ggpubr::ggarrange(mapList[[mapIndex]]) %>%
    ggpubr::ggexport(
      filename = here::here(glue::glue("plots/prediction/1043-det/map_z30_1043Sites_pe{price}.png")),
      width = 2400, height = 1500
    )
...
ggpubr::ggarrange(z_2017_1043Sites,
  ...
  ggpubr::ggexport(
    filename = here::here(glue::glue("plots/prediction/1043-det/map_z0z30GammaTheta_1043Sites_allPrices_det.png")),
...
for (p in seq_along(aux.prices)) {
  ...
  ggpubr::ggarrange(z_2017_1043Sites, aux.mapList[[2]], aux.mapList[[3]],
    ...
    ggpubr::ggexport(
      filename = here::here(glue::glue("plots/prediction/1043-det/map_zDecades_1043Sites_pe{aux.prices[p]}_det.png")),
      width = 2700, height = 1500
    )
}
...
for (y in seq_along(aux.years)) {
  ...
  plot <- ggplot2::ggplot(data = prediction.1043SitesModel %>%
    ...
    ggplot2::geom_sf(aes(fill = z_t)) +
    ggplot2::scale_fill_manual(name = expr(paste("Z"[!!aux.years[y]]^"i", ~"(%), ", "P"^"e", "=", 23.0)), values = c("white", RColorBrewer::brewer.pal(5, "YlOrRd")), drop = FALSE) +
    ggplot2::geom_sf(data = clean.amazonBiome, fill = NA, color = "darkgreen", size = 1.2) +
    ggplot2::guides(fill = guide_legend(label.position = "bottom", title.position = "top", nrow = 1)) +
    ggplot2::theme(
      panel.grid.major = element_line(colour = "white"),
  ...
  ggplot2::ggsave(here::here(glue::glue("plots/prediction/1043-det/map_z{aux.years[y]}_1043Sites_pe20.76.png")), plot = plot, width = 12, height = 9, dpi = 300)
}
