...
aux.prices <- c(6.9, 16.9, 21.9, 26.9, 31.9)
# aux.prices <- c(6.9)
...
prediction.78SitesModel <- purrr::map_df(
  ...
  .f = ~ readr::read_delim(
    here::here(
      "data/prediction/78-mpc",
      glue::glue("p_e_{.x}/amazon_data_z.dat")
    ),
    skip = 1,
    col_types = cols(.default = "n"), col_names = c("Y", "pa", "z_t")
  ) %>%
    dplyr::mutate(p_e = .x)
) %>%
  # dplyr::mutate(id = c(1:78, rep(1:78, times = 199)))
  dplyr::mutate(id = c(1:78, rep(1:78, times = 199), 1:78, rep(1:78, times = 199), 1:78, rep(1:78, times = 199), 1:78, rep(1:78, times = 199), 1:78, rep(1:78, times = 199)))
...
# z_2017_78Sites
z_2017_78Sites <-
  ggplot2::ggplot(data = prediction.78SitesModel %>% dplyr::filter(time == 1, p_e == 6.9) %>%
    dplyr::mutate(z_2017_78Sites = cut(round(z_2017_78Sites, 2),
...
zNumber_2017_78Sites <-
  ...
  ggplot2::geom_text(
    data = prediction.78SitesModel %>%
      dplyr::filter(time == 50, p_e == 6.9, rank_theta_78Sites == 8, rank_gamma_78Sites == 78) %>%
      dplyr::mutate(z_2017_78Sites = round(z_2017_78Sites)),
...
for (price in aux.prices) {
  ...
  ggpubr::ggarrange(zNumber_2017_78Sites, mapList[[mapIndex]], gammaNumber_78Sites, thetaNumber_78Sites, ncol = 2, nrow = 2) %>%
    ggpubr::ggexport(
      filename = here::here(glue::glue("plots/prediction/78-MPC/map_z0z50GammaThetaNumber_78Sites_pe{price}.png")),
...
for (price in aux.prices) {
  ...
  ggpubr::ggarrange(zNumber_2017_78Sites, mapList[[mapIndex]], gammaNumber_78Sites, thetaNumber_78Sites, ncol = 2, nrow = 2) %>%
    ggpubr::ggexport(
      filename = here::here(glue::glue("plots/prediction/78-MPC/map_z0z30GammaThetaNumber_78Sites_pe{price}.png")),
      width = 2400, height = 1500
    )
...
for (price in aux.prices) {
  ...
  ggpubr::ggarrange(mapList[[mapIndex]]) %>%
    ggpubr::ggexport(
      filename = here::here(glue::glue("plots/prediction/78-MPC/map_z50_78Sites_pe{price}.png")),
      width = 2400, height = 1500
    )
...
ggpubr::ggarrange(gamma_78Sites, theta_78Sites, z_2017_78Sites, "",
  ...
  ggpubr::ggexport(
    filename = here::here(glue::glue("plots/prediction/78-MPC/map_z0z50GammaTheta_78Sites_allPrices_mpc.png")),
...
for (price in aux.prices) {
  ...
  ggpubr::ggarrange(mapList[[mapIndex]]) %>%
    ggpubr::ggexport(
      filename = here::here(glue::glue("plots/prediction/78-MPC/map_z30_78Sites_pe{price}.png")),
      width = 2400, height = 1500
    )
...
ggpubr::ggarrange(z_2017_78Sites,
  ...
  ggpubr::ggexport(
    filename = here::here(glue::glue("plots/prediction/78-MPC/map_z0z30GammaTheta_78Sites_allPrices_mpc.png")),
    width = 2700, height = 1500
  )
...
for (p in seq_along(aux.prices)) {
  ...
  ggpubr::ggarrange(z_2017_78Sites, aux.mapList[[2]], aux.mapList[[3]],
    ...
    ggpubr::ggexport(
      filename = here::here(glue::glue("plots/prediction/78-MPC/map_zDecades_78Sites_pe{aux.prices[p]}_mpc.png")),
      width = 2700, height = 1500
    )
}
...
for (y in seq_along(aux.years)) {
  # z50 map (vary by model)
  plot <- ggplot2::ggplot(data = prediction.78SitesModel %>%
    dplyr::filter(time == aux.years[y] - 2017, p_e == 21.9) %>%
    dplyr::mutate(z_t = cut(round(z_t, 5),
      breaks = c(0, 0.5, 20, 40, 60, 80, 105),
      include.lowest = T,
      dig.lab = 3,
  ...
  ggplot2::ggsave(here::here(glue::glue("plots/prediction/78-MPC/map_z{aux.years[y]}_78Sites_pe21.9.png")), plot = plot, width = 12, height = 9, dpi = 300)
}
