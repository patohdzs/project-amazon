...
calibration.78SitesModel <-
  calibration.78SitesModel %>%
  dplyr::mutate(
    rank_theta_78Sites = dense_rank(desc(theta_78Sites)), # rank values
    rank_gamma_78Sites = dense_rank(desc(gamma_78Sites))
  ) %>% # rank values
...
prediction.78SitesModel <-
  ...
  sf::st_as_sf() %>%
  dplyr::mutate(
    z_t = 100 * 1e11 * z_t / zbar_2017_78Sites,
    z_2017_78Sites = 100 * z_2017_78Sites / zbar_2017_78Sites
  ) # transform share to %
...
# gamma_78Sites
gamma_78Sites <-
  ggplot2::ggplot(data = prediction.78SitesModel %>% dplyr::filter(time == 0, p_e == 7.1) %>%
...
for (price in aux.prices) {
  ...
  ggpubr::ggarrange(mapList[[mapIndex]]) %>%
    ggpubr::ggexport(
      filename = here::here(glue::glue("plots/prediction/78-det/map_z50_78Sites_pe{price}.png")),
...
ggpubr::ggarrange(gamma_78Sites, theta_78Sites, z_2017_78Sites, "",
  ...
  ggpubr::ggexport(
    filename = here::here(glue::glue("plots/prediction/78-det/map_z0z50GammaTheta_78Sites_allPrices_det.png")),
    width = 2700, height = 1500
  )

###### 30 years
# LOOP ACROSS PRICES TO GENERATE Z30 MAP AND SAVE Z0, Z30, GAMMA, AND THETA MAPS BY PRICE
mapList <- list()
mapIndex <- 1
for (price in aux.prices) {
  transfer <- price - 7.1
  ...
  ggpubr::ggarrange(mapList[[mapIndex]]) %>%
    ggpubr::ggexport(
      filename = here::here(glue::glue("plots/prediction/78-det/map_z30_78Sites_pe{price}.png")),
      width = 2400, height = 1500
    )
...
# FIGURE WTIH ALL MAPS
# SAVE MAP Z0, GAMMA, THETA, with varying prices
ggpubr::ggarrange(z_2017_78Sites,
  mapList[[1]], mapList[[2]], mapList[[5]],
  ncol = 4, nrow = 1
) %>%
  ggpubr::ggexport(
    filename = here::here(glue::glue("plots/prediction/78-det/map_z0z30GammaTheta_78Sites_allPrices_det.png")),
...
for (price in aux.prices) {
  ...
  mapList[[mapIndex]] <-
    ggplot2::ggplot(data = prediction.78SitesModel %>%
      dplyr::filter(time == 15, p_e == price) %>%
      dplyr::mutate(z_t = cut(round(z_t, 5),
        breaks = c(0, 0.5, 20, 40, 60, 80, 105),
        include.lowest = T,
        dig.lab = 3,
    ...
    ggplot2::geom_sf(aes(fill = z_t)) +
    ggplot2::scale_fill_manual(name = expr(paste("Z"[2032]^"i", ~"(%), ", "b", "=", !!transfer)), values = c("white", RColorBrewer::brewer.pal(5, "YlOrRd")), drop = FALSE) +
    ggplot2::geom_sf(data = clean.amazonBiome, fill = NA, color = "darkgreen", size = 1.2) +
    ggplot2::guides(fill = guide_legend(label.position = "bottom", title.position = "top", nrow = 1)) +
    ggplot2::theme(
      panel.grid.major = element_line(colour = "white"),
  ...
  ggpubr::ggarrange(mapList[[mapIndex]]) %>%
    ggpubr::ggexport(
      filename = here::here(glue::glue("plots/prediction/78-det/map_z15_78Sites_pe{price}.png")),
      width = 2400, height = 1500
    )
...
ggpubr::ggarrange(gamma_78Sites, theta_78Sites, z_2017_78Sites, "",
  ...
  ggpubr::ggexport(
    filename = here::here(glue::glue("plots/prediction/78-det/map_z0z15GammaTheta_78Sites_allPrices_det.png")),
    width = 2700, height = 1500
  )
...
for (p in seq_along(aux.prices)) {
  ...
  ggpubr::ggarrange(z_2017_78Sites, aux.mapList[[2]], aux.mapList[[3]],
    ...
    ggpubr::ggexport(
      filename = here::here(glue::glue("plots/prediction/78-det/map_zDecades_78Sites_pe{aux.prices[p]}_det.png")),
      width = 2700, height = 1500
    )
}
...
for (y in seq_along(aux.years)) {
  # z50 map (vary by model)
  ggplot2::ggplot(data = prediction.78SitesModel %>%
    dplyr::filter(time == aux.years[y] - 2017, p_e == 22.1) %>%
    dplyr::mutate(z_t = cut(round(z_t, 5),
      breaks = c(0, 0.5, 20, 40, 60, 80, 105),
      include.lowest = T,
      dig.lab = 3,
  ...
  ggplot2::ggsave(here::here(glue::glue("plots/prediction/78-det/map_z{aux.years[y]}_78Sites_pe20.76.png")), plot = plot, width = 12, height = 9, dpi = 300)
}

####### Movie
# LOOP ACROSS YEARS TO GENERATE Z DYNAMICS IN MULTIPLE FIGURES WITH PRICE = 20.76
# years by decade
# aux.years <- seq(from = 2017, to = 2047, by = 1)
...
#                                                 labels = c("[0]", "(0-20]", "(20-40]", "(40-60]", "(60-80]", "(80-100]")))) +
#       ggplot2::geom_sf(aes(fill = z_t)) +
#       ggplot2::scale_fill_manual(name = expr(paste("Z"[!!aux.years[y]]^"i", ~"(%), ", "b", "=", 15)), values = c("white", RColorBrewer::brewer.pal(5, "YlOrRd")), drop = FALSE) +
#       ggplot2::geom_sf(data = clean.amazonBiome, fill = NA, color = "darkgreen", size = 1.2) +
#       ggplot2::guides(fill = guide_legend(label.position = "bottom", title.position = "top", nrow = 1)) +
...
#     print(plot)
#   }
# }, movie.name = "movie_b15.gif", interval = 0.5, ani.width = 800, ani.height = 480)

# saveGIF({
#   for (y in seq_along(aux.years)) {
#       plot  <-
#       ggplot2::ggplot(data = prediction.78SitesModel %>%
#                         dplyr::filter(time == aux.years[y]-2017, p_e == 7.1) %>%
#                         dplyr::mutate(z_t = cut(round(z_t,5),
#                                                 breaks = c(0,0.5,20,40,60,80,105),
#                                                 include.lowest = T,
#                                                 dig.lab = 3,
#                                                 labels = c("[0]", "(0-20]", "(20-40]", "(40-60]", "(60-80]", "(80-100]")))) +
#       ggplot2::geom_sf(aes(fill = z_t)) +
#       ggplot2::scale_fill_manual(name = expr(paste("Z"[!!aux.years[y]]^"i", ~"(%), ", "b", "=", 0)), values = c("white", RColorBrewer::brewer.pal(5, "YlOrRd")), drop = FALSE) +
#       ggplot2::geom_sf(data = clean.amazonBiome, fill = NA, color = "darkgreen", size = 1.2) +
#       ggplot2::guides(fill = guide_legend(label.position = "bottom", title.position = "top", nrow = 1)) +
#       ggplot2::theme(panel.grid.major = element_line(colour = "white"),
#                     panel.grid.minor = element_line(colour = "white"),
...
#     print(plot)
#   }
# }, movie.name = "movie_b0.gif", interval = 0.5, ani.width = 800, ani.height = 480)
...
saveGIF(
  {
    for (y in seq_along(aux.years)) {
      ...
      combined_plot <- plot_b0 + plot_b15 + plot_layout(ncol = 2, widths = c(1.5, 1.5)) + plot_annotation(
        title = "Z_t(%) - Percentage Agricultural Area Over Years \n b - Transfers ($/ton of CO2)",
        theme = theme(plot.title = element_text(hjust = 1, size = 24, face = "bold"))
      )
      print(combined_plot)
    }
  },
  movie.name = "movie_78site.gif",
  interval = 0.5,
  ani.width = 1650,
  ani.height = 650
)
