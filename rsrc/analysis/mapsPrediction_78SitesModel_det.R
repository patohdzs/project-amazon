...
prediction.78SitesModel <- purrr::map_df(.x = aux.prices,
                                           .f = ~readr::read_delim(here::here("data/prediction/78-det",
...
for (price in aux.prices) {
  ...
  mapList[[mapIndex]] <-
    ...
    ggpubr::ggarrange(mapList[[mapIndex]]) %>%
      ggpubr::ggexport(filename = here::here(glue::glue("results/prediction/78-det/map_z50_78Sites_pe{price}.png")),
...
ggpubr::ggarrange(gamma_78Sites, theta_78Sites, z_2017_78Sites, "",
                  ...
                  ncol = 4, nrow = 2) %>%
  ggpubr::ggexport(filename = here::here(glue::glue("results/prediction/78-det/map_z0z50GammaTheta_78Sites_allPrices_det.png")),
...
for (price in aux.prices) {
  ...
  mapList[[mapIndex]] <-
    ...
    # SAVE MAP Z0, Z50, GAMMA, THETA
    ggpubr::ggarrange(mapList[[mapIndex]]) %>%
      ggpubr::ggexport(filename = here::here(glue::glue("results/prediction/78-det/map_z30_78Sites_pe{price}.png")),
                       width = 2400, height = 1500)
...
ggpubr::ggarrange( z_2017_78Sites, 
                  ...
                  ncol = 4, nrow = 1) %>%
  ggpubr::ggexport(filename = here::here(glue::glue("results/prediction/78-det/map_z0z30GammaTheta_78Sites_allPrices_det.png")),
...
for (price in aux.prices) {
  ...
  mapList[[mapIndex]] <-
    ...
    # SAVE MAP Z0, Z50, GAMMA, THETA
    ggpubr::ggarrange(mapList[[mapIndex]]) %>%
      ggpubr::ggexport(filename = here::here(glue::glue("results/prediction/78-det/map_z15_78Sites_pe{price}.png")),
                       width = 2400, height = 1500)
...
ggpubr::ggarrange(gamma_78Sites, theta_78Sites, z_2017_78Sites, "",
                  mapList[[1]], mapList[[2]], mapList[[3]], mapList[[4]],
                  ncol = 4, nrow = 2) %>%
  ggpubr::ggexport(filename = here::here(glue::glue("results/prediction/78-det/map_z0z15GammaTheta_78Sites_allPrices_det.png")),
                   width = 2700, height = 1500)
...
for (p in seq_along(aux.prices)) {
  ...
  ggpubr::ggarrange(z_2017_78Sites, aux.mapList[[2]], aux.mapList[[3]],
                    ...
                    ncol = 3, nrow = 2) %>%
    ggpubr::ggexport(filename = here::here(glue::glue("results/prediction/78-det/map_zDecades_78Sites_pe{aux.prices[p]}_det.png")),
                     width = 2700, height = 1500)
...
for (y in seq_along(aux.years)) {
  ...
  # z50 map (vary by model)
    ...
    ggplot2::theme(panel.grid.major = element_line(colour = "white"),
                   ...
                   legend.text = element_text(size = 28, face = "bold"),
                   plot.margin=unit(c(t=-0.5,r=-1,b=-0,l=-1), "cm"))
  ggplot2::ggsave(here::here(glue::glue("results/prediction/78-det/map_z{aux.years[y]}_78Sites_pe20.76.png")), width = 12, height = 9, dpi = 300)
...
}, movie.name = "movie_78site.gif", interval = 0.5, ani.width = 1650, ani.height = 650)
