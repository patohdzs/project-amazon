...
for (price in aux.prices) {
  ...
  ggpubr::ggarrange(mapList[[mapIndex]]) %>%
    ggpubr::ggexport(
      filename = here::here(glue::glue("plots/prediction/78-hmc/map_z50_78Sites_pe{price}.png")),
...
ggpubr::ggarrange(gamma_78Sites, theta_78Sites, z_2017_78Sites, "",
  ...
  ggpubr::ggexport(
    filename = here::here(glue::glue("plots/prediction/78-hmc/map_z0z50GammaTheta_78Sites_allPrices_det.png")),
...
for (p in seq_along(aux.prices)) {
  ...
  ggpubr::ggarrange(z_2017_78Sites, aux.mapList[[2]], aux.mapList[[3]],
    ...
    ggpubr::ggexport(
      filename = here::here(glue::glue("plots/prediction/78-hmc/map_zDecades_78Sites_pe{aux.prices[p]}_det.png")),
      width = 2700, height = 1500
    )
}
...
for (y in seq_along(aux.years)) {
  ...
  ggplot2::ggsave(here::here(glue::glue("plots/prediction/78-hmc/map_z{aux.years[y]}_78Sites_pe20.76.png")), plot = plot, width = 12, height = 9, dpi = 300)
}
