...
z_2017_78Sites <-
  ggplot2::ggplot(data = prediction.78SitesModel %>% dplyr::filter(time == 1, p_e == 6.5) %>%
                    dplyr::mutate(z_2017_78Sites = cut(round(z_2017_78Sites, 2), breaks = c(0,0.01,20,40,60,80,105), include.lowest = T, dig.lab = 3,
...
for (price in aux.prices) {
  ...
  ggpubr::ggarrange(zNumber_2017_78Sites, mapList[[mapIndex]], gammaNumber_78Sites, thetaNumber_78Sites, ncol = 2, nrow = 2) %>%
    ggpubr::ggexport(filename = here::here(glue::glue("results/prediction/78SitesModel/map_z0z50GammaThetaNumber_78Sites_pe{price}.png")),
...
for (price in aux.prices) {
  ...
  ggpubr::ggarrange(mapList[[mapIndex]]) %>%
    ggpubr::ggexport(filename = here::here(glue::glue("results/prediction/78SitesModel/map_z50_78Sites_pe{price}.png")),
                     width = 2400, height = 1500)
...
ggpubr::ggarrange(gamma_78Sites, theta_78Sites, z_2017_78Sites, "",
                  ...
                  ncol = 4, nrow = 2) %>%
  ggpubr::ggexport(filename = here::here(glue::glue("results/prediction/78SitesModel/map_z0z50GammaTheta_78Sites_allPrices.png")),
...
for (p in seq_along(aux.prices)) {
  ...
  ggpubr::ggarrange(z_2017_78Sites, aux.mapList[[2]], aux.mapList[[3]],
                    ...
                    ncol = 3, nrow = 2) %>%
    ggpubr::ggexport(filename = here::here(glue::glue("results/prediction/78SitesModel/map_zDecades_78Sites_pe{aux.prices[p]}.png")),
                     width = 2700, height = 1500)
...
for (y in seq_along(aux.years)) {
  ...
  ggplot2::ggplot(data = prediction.78SitesModel %>%
    ...
    ggplot2::theme(panel.grid.major = element_line(colour = "white"),
                   ...
                   legend.text = element_text(size = 28, face = "bold"),
                   plot.margin=unit(c(t=-0.5,r=-1,b=-0,l=-1), "cm"))
  ggplot2::ggsave(here::here(glue::glue("results/prediction/78SitesModel/map_z{aux.years[y]}_78Sites_pe20.76.png")), width = 12, height = 9, dpi = 300)
