...
# z_2017_78Sites
z_2017_78Sites <-
  ggplot2::ggplot(data = prediction.78SitesModel %>% dplyr::filter(time == 1, p_e == 6.5) %>%
    dplyr::mutate(z_2017_78Sites = cut(round(z_2017_78Sites, 2),
      breaks = c(0, 0.01, 20, 40, 60, 80, 105), include.lowest = T, dig.lab = 3,
...
zNumber_2017_78Sites <-
  ...
  ggplot2::geom_text(
    data = prediction.78SitesModel %>%
      dplyr::filter(time == 50, p_e == 6.5, rank_theta_78Sites == 8, rank_gamma_78Sites == 78) %>%
      dplyr::mutate(z_2017_78Sites = round(z_2017_78Sites)),
...
for (price in aux.prices) {
  ...
  mapList[[mapIndex]] <-
    ...
    ggplot2::ggtitle(expr(paste("Z"[2067]^"i", ~"(%), ", "P"^"e", "=", !!price))) +
    ggplot2::theme(
      panel.grid.major = element_line(colour = "White"),
      panel.grid.minor = element_line(colour = "white"),
      panel.background = element_blank(),
  ...
  ggpubr::ggarrange(zNumber_2017_78Sites, mapList[[mapIndex]], gammaNumber_78Sites, thetaNumber_78Sites, ncol = 2, nrow = 2) %>%
    ggpubr::ggexport(
      filename = here::here(glue::glue("plots/prediction/78SitesModel/map_z0z50GammaThetaNumber_78Sites_pe{price}.png")),
...
for (price in aux.prices) {
  ...
  ggpubr::ggarrange(mapList[[mapIndex]]) %>%
    ggpubr::ggexport(
      filename = here::here(glue::glue("plots/prediction/78SitesModel/map_z50_78Sites_pe{price}.png")),
      width = 2400, height = 1500
    )
...
ggpubr::ggarrange(gamma_78Sites, theta_78Sites, z_2017_78Sites, "",
  ...
  ggpubr::ggexport(
    filename = here::here(glue::glue("plots/prediction/78SitesModel/map_z0z50GammaTheta_78Sites_allPrices.png")),
...
for (p in seq_along(aux.prices)) {
  ...
  ggpubr::ggarrange(z_2017_78Sites, aux.mapList[[2]], aux.mapList[[3]],
    ...
    ggpubr::ggexport(
      filename = here::here(glue::glue("plots/prediction/78SitesModel/map_zDecades_78Sites_pe{aux.prices[p]}.png")),
      width = 2700, height = 1500
    )
}
...
for (y in seq_along(aux.years)) {
  ...
  plot <- ggplot2::ggplot(data = prediction.78SitesModel %>%
    dplyr::filter(time == aux.years[y] - 2017, p_e == 20.76) %>%
    dplyr::mutate(z_t = cut(round(z_t, 5),
      breaks = c(0, 0.01, 20, 40, 60, 80, 105),
      include.lowest = T,
      dig.lab = 3,
    ...
    ggplot2::geom_sf(aes(fill = z_t)) +
    ggplot2::scale_fill_manual(name = expr(paste("Z"[!!aux.years[y]]^"i", ~"(%), ", "P"^"e", "=", 20.76)), values = c("white", RColorBrewer::brewer.pal(5, "YlOrRd")), drop = FALSE) +
    ggplot2::geom_sf(data = clean.amazonBiome, fill = NA, color = "darkgreen", size = 1.2) +
    ggplot2::guides(fill = guide_legend(label.position = "bottom", title.position = "top", nrow = 1)) +
    ggplot2::theme(
      panel.grid.major = element_line(colour = "white"),
  ...
  ggplot2::ggsave(here::here(glue::glue("plots/prediction/78SitesModel/map_z{aux.years[y]}_78Sites_pe20.76.png")), plot = plot, width = 12, height = 9, dpi = 300)
}
