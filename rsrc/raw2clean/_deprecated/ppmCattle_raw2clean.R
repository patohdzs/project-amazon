
# > PROJECT INFO
# NAME: INCENTIVES AMAZON
# LEAD: JULIANO ASSUNCAO, JOSE SCHEINKMAN, AND LARS HANSEN
#
# > THIS SCRIPT
# AIM: CLEAN RAW LIVESTOCK CATTLE (HEAD COUNT) - PPM (IBGE)
# AUTHOR: JOAO VIEIRA
#
# > NOTES
# 1: -





# SETUP ----------------------------------------------------------------------------------------------------------------------------------------------

# GROUNDHOG (REPRODUCIBILITY SOLUTION TO HANDLING DIFFERENT VERSIONS OF R AND ITS PACKAGES)

# check if groundhog is installed and load it
if ("groundhog" %in% installed.packages()) {
  library("groundhog")
} else {
  install.packages("groundhog")
  library("groundhog")
}

# define date of reference to load all packages
groundhog.date <- "2022-04-01"

# guarantee version 1.5 of groundhog is being used
groundhog::meta.groundhog(date = "2022-04-01")


# HERE
groundhog::groundhog.library("here", groundhog.date) # load package here


# TICTOC
groundhog::groundhog.library("tictoc", groundhog.date) # load package tictoc


# DECLARE LOCATION OF CURRENT SCRIPT TO SET UP PROJECT ROOT CORRECTLY
here::i_am("code/raw2clean/ppmCattle_raw2clean.R", uuid = "eb3955a6-f041-436a-a011-19ff5d34d9aa")


# START TIME
tictoc::tic(msg = "ppmCattle_raw2clean script", log = T)


# SOURCE FUNCTIONS
source(here::here("code/_functions/ExportTimeProcessing.R"))


# LIBRARIES
groundhog::groundhog.library("tidyverse", groundhog.date)  # manipulate tables, works with sf
groundhog::groundhog.library("sjlabelled", groundhog.date) # label columns, preferred than Hmisc::label because has function to clear labels when necessary





# DATA INPUT -----------------------------------------------------------------------------------------------------------------------------------------

# read csv file
raw.cattle <-  readr::read_csv(file = here::here("data/raw2clean/ppmCattle_ibge/input/ppm_livestock_cattle.csv"),
                               skip = 4,
                               na = c("..."),
                               col_names = c("muni_code", "muni_name", "year", "livestock_cattle"),
                               col_types = list("n", "c", "i", "c"))




# DATA EXPLORATION [disabled for speed]
# summary(raw.cattle)
# View(raw.cattle)





# DATASET CLEANUP AND PREP ---------------------------------------------------------------------------------------------------------------------------

# ROW CLEANUP
# remove last rows of the table with notes information - coincides with NAs in year column
raw.cattle <-
  raw.cattle %>%
  dplyr::filter(!is.na(year))

# transform "-" values to "0" as explained in the "ibge_technicalNotes_ppm2019.pdf" file in documentation
raw.cattle <-
  raw.cattle %>%
  dplyr::mutate(livestock_cattle = dplyr::if_else(livestock_cattle == "-", "0", livestock_cattle))

# latin character treatment
raw.cattle <-
  raw.cattle %>%
  dplyr::mutate(dplyr::across(tidyselect:::where(is.character), iconv, from = "UTF-8", to = "ASCII//TRANSLIT"))


# COLUMN CLEANUP

# transform column class
raw.cattle <-
  raw.cattle %>%
  mutate(livestock_cattle = as.numeric(livestock_cattle))




# EXPORT PREP ----------------------------------------------------------------------------------------------------------------------------------------

# sjlabelled::set_labelS
sjlabelled::set_label(raw.cattle$muni_code)        <- "municipality code (7-digit, IBGE)"
sjlabelled::set_label(raw.cattle$muni_name)        <- "municipality name and (state name abbreviation)"
sjlabelled::set_label(raw.cattle$year)             <- "year of reference (calendar or PRODES year)"
sjlabelled::set_label(raw.cattle$livestock_cattle) <- "(calendar year) number of cattle"

# change object name for exportation
clean.ppmCattle <- raw.cattle



# POST-TREATMENT OVERVIEW
# summary(clean.ppmCattle)
# View(clean.ppmCattle)





# EXPORT ---------------------------------------------------------------------------------------------------------------------------------------------

save(clean.ppmCattle,
     file = here::here("data/raw2clean/ppmCattle_ibge/output",
                      "clean_ppmCattle.Rdata"))


# END TIMER
tictoc::toc(log = T)

# export time to csv table
ExportTimeProcessing("raw2clean")





# END OF SCRIPT --------------------------------------------------------------------------------------------------------------------------------------