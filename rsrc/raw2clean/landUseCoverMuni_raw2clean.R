...
raw.mapbiomas <-
  ...
  dplyr::mutate(mapbiomas_id = case_when(mapbiomas_class == "Anthropic_3 - Farming_Agriculture_Temporary Crops_Mosaic of Crops" ~ "mapbiomasLandCoverId_41",
                                  ...
                                  mapbiomas_class == "Natural_2 - Non Forest Natural Formation_Other Non Forest Natural Formation_Other Non Forest Natural Formation_Other Non Forest Natural Formation" ~ "mapbiomasLandCoverId_13")) %>% # add mapbiomas id to simplify future column names - see "_PT-BR__Códigos_da_legenda_Coleção_5" on documentation folder
  ...
  tidyr::pivot_wider(id_cols = c("muni_code", "year"), names_from = "mapbiomas_id",
              values_from = "mapbiomas_id_area", values_fill = list(mapbiomas_id_area = 0)) %>% # transform mapbiomas_id rows to wide format filling NA values with 0
...
save(clean.landUseCoverMuni,
     file = here::here("data/raw2clean/landUseCoverMuni_mapbiomas/output",
