...
calib_df$group_id <- 1:nrow(calib_df)
intersections <- st_intersection(calib_1043, calib_df)

intersections$area <- st_area(intersections)

intersections <- intersections %>%
  group_by(id) %>% # Assuming 'id' is the identifier for 1043-level sites
  mutate(proportional_area = area / sum(area)) %>%
  ungroup()

intersections <- intersections %>%
  group_by(id) %>%
  filter(proportional_area == max(proportional_area)) %>%
  ungroup()
