...
#
# > THIS SCRIPT
# AIM: PARAMETERS CALIBRATION (12 SITES MODEL)
# AUTHOR: JOAO VIEIRA
#
# > NOTES
# 1: -
...
here::i_am("code/projectSpecific/12SitesModel/calibration_projectSpecific_12SitesModel.R", uuid = "0fef2915-0a76-4b7f-9a2c-c0b9830c058b")


# START TIME
tictoc::tic(msg = "calibration_projectSpecific_12SitesModel script", log = T)
...
# GROUP 1000 SITES INTO 12 CLUSTERS
...
aux.cluster <- kmeans(aux.clusterData, centers = 12, nstart = 25)
...
ggplot2::ggsave(filename = here::here("data/analysis/12SitesModel/cluster_gammaTheta_1000Sites.png"), width = 10, height = 6)
...
ggplot2::ggsave(filename = here::here("data/analysis/12SitesModel/map_12clusters_1000Sites.png"), width = 8, height = 6)
...
ggplot2::ggsave(filename = here::here("data/analysis/12SitesModel/map_12clustersRankGamma_1000Sites.png"), width = 8, height = 6)

# intermediate export before aggregating
save(calibration.1000SitesModel,
     file = here::here("data/projectSpecific/1000SitesModel",
                       "cluster12Sites_1000SitesModel.Rdata"))
...
# AGGREGATE FROM 1000 SITES TO 12 SITES
calibration.12SitesModel <-
  calibration.1000SitesModel %>%
  ...
  dplyr::ungroup() %>%
  sf::st_as_sf() %>%
  dplyr::rename_with(.fn = ~str_replace(., "1000Sites", "12Sites"), .cols = ends_with("1000Sites"))

# add id variable
calibration.12SitesModel$id <- 1:nrow(calibration.12SitesModel)
...
#   sf::st_transform(sf::st_crs(calibration.12SitesModel)) %>%
#   dplyr::mutate(co2e_ha = (agb/2)*(44/12)) %>%
#   dplyr::select(co2e_ha)
#
# # match minicells with sites
# aux.gamma <- sf::st_join(calibration.12SitesModel, pixelBiomass.prepData)
#
...
#   sf::st_drop_geometry() %>%
#   dplyr::group_by(id) %>%
#   summarise(gamma_12Sites = mean(co2e_ha, na.rm = T))
#
#
# # add gamma_12Sites to spatial variables
# calibration.12SitesModel <- left_join(calibration.12SitesModel, aux.gamma)
#
# # clean environment
...
# calibration.12SitesModel <-
#   calibration.12SitesModel %>%
#   dplyr::mutate(gamma_alt_12Sites = gamma_12Sites*1.101)
...
pixelBiomass2017.prepData %>%
  ...
  fixest::etable(se = "hetero", replace = T, file = here::here("data/analysis/12SitesModel/reg_gamma.tex"), style.tex = fixest::style.tex(main = "qje"))
...
# match minicells with sites
aux.gamma2017 <- sf::st_join(calibration.12SitesModel, pixelBiomass2017.prepData)

# calculate average carbon density on primary forest areas by site
aux.gamma2017 <- aux.gamma2017 %>% sf::st_drop_geometry() %>% dplyr::group_by(id) %>% summarise(gamma_12Sites = mean(co2e_ha_2017, na.rm = T),
                                                                                                co2e_ha_2017 = mean(co2e_ha_2017, na.rm = T),
                                                                                                gammaSD_12Sites = mean(co2eSD_ha_2017_fitted, na.rm = T),
                                                                                                co2eSD_ha_2017 = mean(co2eSD_ha_2017, na.rm = T),
                                                                                                lon = mean(lon, na.rm = T),
                                                                                                lat = mean(lat, na.rm = T))

# add gamma_12Sites to spatial variables
calibration.12SitesModel <- left_join(calibration.12SitesModel, aux.gamma2017)
...
calibration.12SitesModel <-
  calibration.12SitesModel %>%
  dplyr::mutate(gammaSD_12Sites = predict(reg.carbonUncertainty, .),
                gammaSD_12Sites = exp(gammaSD_12Sites))
...
calibration.12SitesModel <-
  calibration.12SitesModel %>%
  dplyr::mutate(a_12Sites = calibration.globalModel$a_global,
                b_12Sites = a_12Sites*gamma_12Sites*zbar_2017_12Sites)
...
calibration.12SitesModel <-
  calibration.12SitesModel %>%
  dplyr::mutate(k_12Sites = calibration.globalModel$k_global)
...
calibration.12SitesModel <-
  calibration.12SitesModel %>%
  dplyr::mutate(zeta_12Sites = calibration.globalModel$zeta_global,
                zeta_alt_12Sites = calibration.globalModel$zeta_alt_global)
...
calibration.12SitesModel <-
  calibration.12SitesModel %>%
  dplyr::mutate(rho_12Sites = calibration.globalModel$rho_global)
...
calibration.12SitesModel <-
  calibration.12SitesModel %>%
  dplyr::mutate(r_12Sites = calibration.globalModel$r_global)
...
# x_2017_12Sites estimated as in the old way of global model, just considering the stock of carbon stored in forest areas assuming that all forests are primary
calibration.12SitesModel <-
  calibration.12SitesModel %>%
  dplyr::mutate(x_2017_12Sites = gamma_12Sites*forestArea_2017_ha_12Sites)
...
calibration.12SitesModel <-
  calibration.12SitesModel %>%
  dplyr::mutate(C_2017_12Sites = calibration.globalModel$C_2017_global,
                C0bern_2017_12Sites = calibration.globalModel$C0bern_2017_global,
                C1bern_2017_12Sites = calibration.globalModel$C1bern_2017_global,
                C2bern_2017_12Sites = calibration.globalModel$C2bern_2017_global,
                C3bern_2017_12Sites = calibration.globalModel$C3bern_2017_global)
...
calibration.12SitesModel <-
  calibration.12SitesModel %>%
  dplyr::mutate(u_12Sites = calibration.globalModel$u_global,
                u_bern_12Sites = calibration.globalModel$u_bern_global)
...
# change crs to match with calibration.12SitesModel and select variables
muniTheta.prepData <-
  muniTheta.prepData %>%
  sf::st_transform(crs = sf::st_crs(calibration.12SitesModel)) %>%
...
aux.theta <- sf::st_intersection(calibration.12SitesModel, muniTheta.prepData)
...
calibration.12SitesModel <- left_join(calibration.12SitesModel, aux.theta.2017)
...
# add cattleSlaughter_value_ha_fitted and pastureArea_value to spatial variables
calibration.12SitesModel <- left_join(calibration.12SitesModel, aux.theta.2006)

# clean environment
rm(muniTheta.prepData, aux.theta)

# calculate theta_12Sites
calibration.12SitesModel <-
  calibration.12SitesModel %>%
  dplyr::mutate(theta_12Sites = cattleSlaughter_value_ha_fitted/(aux.price.2017),
                theta_scenario2_12Sites = cattleSlaughter_value_ha_scenario2/(aux.price.2017),
                theta_scenario3_12Sites = cattleSlaughter_value_ha_scenario3/(aux.price.2017),
                theta_scenario4_12Sites = cattleSlaughter_value_ha_scenario4/(aux.price.2017),
                theta_scenario5_12Sites = cattleSlaughter_value_ha_scenario5/(aux.price.2017)) %>%
  dplyr::mutate(theta2006_12Sites = cattleSlaughter2006_value_ha_fitted/(aux.price.2006),
                theta2006_scenario2_12Sites = cattleSlaughter2006_value_ha_scenario2/(aux.price.2006),
                theta2006_scenario3_12Sites = cattleSlaughter2006_value_ha_scenario3/(aux.price.2006),
                theta2006_scenario4_12Sites = cattleSlaughter2006_value_ha_scenario4/(aux.price.2006),
                theta2006_scenario5_12Sites = cattleSlaughter2006_value_ha_scenario5/(aux.price.2006)) %>%
...
aux.neighbors <- sf::st_is_within_distance(calibration.12SitesModel, calibration.12SitesModel, dist = 100, remove_self = TRUE)

# impute values for missing thetas based on the average of adjacent neighbors
calibration.12SitesModel <-
  calibration.12SitesModel %>%
  dplyr::mutate(d_imputation_theta_scenario2_12Sites = dplyr::if_else(is.na(theta_scenario2_12Sites), 1, 0),
                theta_scenario2_12Sites = dplyr::if_else(is.na(theta_scenario2_12Sites),
                                                          apply(aux.neighbors, 1, function(i){mean(.$theta_scenario2_12Sites[i], na.rm = TRUE)}),
                                                          theta_scenario2_12Sites),
                d_imputation_theta_scenario3_12Sites = dplyr::if_else(is.na(theta_scenario3_12Sites), 1, 0),
                theta_scenario3_12Sites = dplyr::if_else(is.na(theta_scenario3_12Sites),
                                                          apply(aux.neighbors, 1, function(i){mean(.$theta_scenario3_12Sites[i], na.rm = TRUE)}),
                                                          theta_scenario3_12Sites),
                d_imputation_theta_scenario4_12Sites = dplyr::if_else(is.na(theta_scenario4_12Sites), 1, 0),
                theta_scenario4_12Sites = dplyr::if_else(is.na(theta_scenario4_12Sites),
                                                          apply(aux.neighbors, 1, function(i){mean(.$theta_scenario4_12Sites[i], na.rm = TRUE)}),
                                                          theta_scenario4_12Sites),
                d_imputation_theta_scenario5_12Sites = dplyr::if_else(is.na(theta_scenario5_12Sites), 1, 0),
                theta_scenario5_12Sites = dplyr::if_else(is.na(theta_scenario5_12Sites),
                                                          apply(aux.neighbors, 1, function(i){mean(.$theta_scenario5_12Sites[i], na.rm = TRUE)}),
                                                          theta_scenario5_12Sites))
...
calibration.12SitesModel <-
  calibration.12SitesModel %>%
  dplyr::bind_cols(dplyr::tibble(theta_soybean_12Sites = terra::extract(raster.soybean, terra::vect(calibration.12SitesModel %>% sf::st_transform(4326)), mean, na.rm = T)$soyb200b_yld))
...
# match munis with sites
aux.worker <- sf::st_intersection(calibration.12SitesModel, muniAgCensusCattleRaising.prepData)
...
calibration.12SitesModel <- left_join(calibration.12SitesModel, aux.worker)
...
calibration.12SitesModel <-
  calibration.12SitesModel %>%
  dplyr::mutate(workerPerHa_12Sites = workers_ha_fitted) %>%
...
calibration.12SitesModel <-
  calibration.12SitesModel %>%
  dplyr::mutate(p_2017_12Sites = calibration.globalModel$p_2017_global,
                p_2017_alt_12Sites = calibration.globalModel$p_2017_alt_global,
                pSoybean_2017_12Sites = calibration.globalModel$pSoybean_2017_global,
                pSoybean_2017_alt_12Sites = calibration.globalModel$pSoybean_2017_alt_global)
...
calibration.12SitesModel <-
  calibration.12SitesModel %>%
  dplyr::filter(!is.na(gamma_12Sites))


# ORDER VARIABLES
calibration.12SitesModel <-
  calibration.12SitesModel %>%
  dplyr::select(id, ends_with("ha_12Sites"), cluster, clusterRank,
                z_2017_12Sites, zbar_2017_12Sites, x_2017_12Sites, b_12Sites, theta_12Sites, gamma_12Sites, gammaSD_12Sites,
                d_theta_winsorized,
                pastureArea_value, ends_with("_12Sites"))


# POST-TREATMENT OVERVIEW
# summary(calibration.12SitesModel)
# View(calibration.12SitesModel)
...
save(calibration.12SitesModel,
     file = here::here("data/projectSpecific/12SitesModel",
                       "calibration_12SitesModel.Rdata"))

# remove spatial feature
calibration.12SitesModel <- calibration.12SitesModel %>% sf::st_drop_geometry()

readr::write_csv(calibration.12SitesModel,
                 file = here::here("data/projectSpecific/12SitesModel", "calibration_12SitesModel.csv"))
...
ExportTimeProcessing("projectSpecific/12SitesModel")
