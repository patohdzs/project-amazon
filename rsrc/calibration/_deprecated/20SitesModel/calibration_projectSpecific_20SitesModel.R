...
#
# > THIS SCRIPT
# AIM: PARAMETERS CALIBRATION (20 SITES MODEL)
# AUTHOR: JOAO VIEIRA
#
# > NOTES
# 1: -
...
here::i_am("code/projectSpecific/20SitesModel/calibration_projectSpecific_20SitesModel.R", uuid = "0fef2915-0a76-4b7f-9a2c-c0b9830c058b")


# START TIME
tictoc::tic(msg = "calibration_projectSpecific_20SitesModel script", log = T)
...
groundhog::groundhog.library("Hmisc", groundhog.date)  # wtd.var
...
# GROUP 1000 SITES INTO 10 CLUSTERS
...
aux.cluster <- kmeans(aux.clusterData, centers = 20, nstart = 25)
...
fviz_cluster(aux.cluster, data = aux.clusterData, pointsize = 3, stand = F, geom = "text", show.clust.cent = F) +
  ...
  ggplot2::scale_color_viridis_d(name = "Cluster") +
...
ggplot2::ggsave(filename = here::here("data/analysis/20SitesModel/cluster_gammaTheta_1000Sites.png"), width = 10, height = 6)

# check optimal number of clusters
fviz_nbclust(aux.clusterData, kmeans, method = "wss", k.max = 25)
fviz_nbclust(aux.clusterData, kmeans, method = "silhouette", k.max = 25)
...
ggplot2::ggplot(data = calibration.1000SitesModel) +
  ...
  ggplot2::theme(panel.grid.major = element_line(colour = "White"),
                 ...
                 legend.position = "bottom", legend.key.width = unit(0.2, "cm"), legend.margin=margin(t=-0.5, r=0, b=0.2, l=0, unit="cm"),
                 legend.text = element_text(size = 12, face = "bold"))

ggplot2::ggsave(filename = here::here("data/analysis/20SitesModel/map_20clustersRankGamma_1000Sites.png"), width = 8, height = 6)

# intermediate export before aggregating
save(calibration.1000SitesModel,
     file = here::here("data/projectSpecific/1000SitesModel",
                       "cluster20Sites_1000SitesModel.Rdata"))
...
# AGGREGATE FROM 1000 SITES TO 20 SITES
calibration.20SitesModel <-
  calibration.1000SitesModel %>%
  ...
  dplyr::ungroup() %>%
  sf::st_as_sf() %>%
  dplyr::rename_with(.fn = ~str_replace(., "1000Sites", "20Sites"), .cols = ends_with("1000Sites"))


ggplot2::ggplot(data = calibration.20SitesModel) +
  ggplot2::geom_sf(aes(fill = as.factor(clusterRank))) +
  ggplot2::scale_fill_viridis_d(name = "Cluster") +
  ggplot2::guides(fill = guide_legend(label.position = "bottom", title.position = "top", nrow = 1)) +
  ggplot2::theme(panel.grid.major = element_line(colour = "White"),
...
# add id variable
calibration.20SitesModel$id <- 1:nrow(calibration.20SitesModel)
...
#   sf::st_transform(sf::st_crs(calibration.20SitesModel)) %>%
#   dplyr::mutate(co2e_ha = (agb/2)*(44/12)) %>%
#   dplyr::select(co2e_ha)
#
# # match minicells with sites
# aux.gamma <- sf::st_join(calibration.20SitesModel, pixelBiomass.prepData)
#
...
#   sf::st_drop_geometry() %>%
#   dplyr::group_by(id) %>%
#   summarise(gamma_20Sites = mean(co2e_ha, na.rm = T))
#
#
# # add gamma_20Sites to spatial variables
# calibration.20SitesModel <- left_join(calibration.20SitesModel, aux.gamma)
#
# # clean environment
...
# calibration.20SitesModel <-
#   calibration.20SitesModel %>%
#   dplyr::mutate(gamma_alt_20Sites = gamma_20Sites*1.101)
...
pixelBiomass2017.prepData %>%
  ...
  fixest::etable(se = "hetero", replace = T, file = here::here("data/analysis/20SitesModel/reg_gamma.tex"), style.tex = fixest::style.tex(main = "qje"))
...
# match minicells with sites
aux.gamma2017 <- sf::st_join(calibration.20SitesModel, pixelBiomass2017.prepData)

# calculate average carbon density on primary forest areas by site
aux.gamma2017 <- aux.gamma2017 %>% sf::st_drop_geometry() %>% dplyr::group_by(id) %>% summarise(gamma_20Sites = mean(co2e_ha_2017, na.rm = T),
                                                                                                co2e_ha_2017 = mean(co2e_ha_2017, na.rm = T),
                                                                                                gammaSD_20Sites = mean(co2eSD_ha_2017_fitted, na.rm = T),
                                                                                                co2eSD_ha_2017 = mean(co2eSD_ha_2017, na.rm = T),
                                                                                                lon = mean(lon, na.rm = T),
                                                                                                lat = mean(lat, na.rm = T))

# add gamma_20Sites to spatial variables
calibration.20SitesModel <- left_join(calibration.20SitesModel, aux.gamma2017)
...
calibration.20SitesModel <-
  calibration.20SitesModel %>%
  dplyr::mutate(gammaSD_20Sites = predict(reg.carbonUncertainty, .),
                gammaSD_20Sites = exp(gammaSD_20Sites))
...
calibration.20SitesModel <-
  calibration.20SitesModel %>%
  dplyr::mutate(a_20Sites = calibration.globalModel$a_global,
                b_20Sites = a_20Sites*gamma_20Sites*zbar_2017_20Sites)
...
calibration.20SitesModel <-
  calibration.20SitesModel %>%
  dplyr::mutate(k_20Sites = calibration.globalModel$k_global)
...
calibration.20SitesModel <-
  calibration.20SitesModel %>%
  dplyr::mutate(zeta_20Sites = calibration.globalModel$zeta_global,
                zeta_alt_20Sites = calibration.globalModel$zeta_alt_global)
...
calibration.20SitesModel <-
  calibration.20SitesModel %>%
  dplyr::mutate(rho_20Sites = calibration.globalModel$rho_global)
...
calibration.20SitesModel <-
  calibration.20SitesModel %>%
  dplyr::mutate(r_20Sites = calibration.globalModel$r_global)
...
# x_2017_20Sites estimated as in the old way of global model, just considering the stock of carbon stored in forest areas assuming that all forests are primary
calibration.20SitesModel <-
  calibration.20SitesModel %>%
  dplyr::mutate(x_2017_20Sites = gamma_20Sites*forestArea_2017_ha_20Sites)
...
calibration.20SitesModel <-
  calibration.20SitesModel %>%
  dplyr::mutate(C_2017_20Sites = calibration.globalModel$C_2017_global,
                C0bern_2017_20Sites = calibration.globalModel$C0bern_2017_global,
                C1bern_2017_20Sites = calibration.globalModel$C1bern_2017_global,
                C2bern_2017_20Sites = calibration.globalModel$C2bern_2017_global,
                C3bern_2017_20Sites = calibration.globalModel$C3bern_2017_global)
...
calibration.20SitesModel <-
  calibration.20SitesModel %>%
  dplyr::mutate(u_20Sites = calibration.globalModel$u_global,
                u_bern_20Sites = calibration.globalModel$u_bern_global)
...
# change crs to match with calibration.20SitesModel and select variables
muniTheta.prepData <-
  muniTheta.prepData %>%
  sf::st_transform(crs = sf::st_crs(calibration.20SitesModel)) %>%
  dplyr::select(muni_code, muni_area, cattleSlaughter_value_ha_fitted, cattleSlaughter_value_ha_resid,
                cattleSlaughter_farmGatePrice_fitted, pastureArea_value, d_theta_winsorized)

# match munis with sites
aux.theta <- sf::st_intersection(calibration.20SitesModel, muniTheta.prepData)
...
aux.theta %>%
  sf::st_drop_geometry() %>%
  dplyr::mutate(theta_resid_muni = cattleSlaughter_value_ha_resid/(aux.price.2017),
                theta_muni = cattleSlaughter_value_ha_fitted/(aux.price.2017)) %>%
  dplyr::select(id, muni_code, theta_resid_muni, theta_muni, muni_area, muni_site_area, pastureArea_value) %>%
  readr::write_csv(here::here("data/projectSpecific/20SitesModel", "20site_muni.csv"))
...
aux.theta.2017 <-
  ...
  sf::st_drop_geometry() %>%
  dplyr::group_by(id) %>%
  dplyr::summarise(theta_20Sites = weighted.mean(cattleSlaughter_value_ha_fitted/(aux.price.2017), w = pastureArea_value*(muni_site_area/muni_area), na.rm = T),
                   thetaSD_20Sites = sqrt(Hmisc::wtd.var(cattleSlaughter_value_ha_fitted/(aux.price.2017), w = pastureArea_value*(muni_site_area/muni_area), na.rm = T)),
                   theta_residSD_20Sites = sqrt(Hmisc::wtd.var(cattleSlaughter_value_ha_resid/(aux.price.2017), w = pastureArea_value*(muni_site_area/muni_area), na.rm = T)),
                   theta_resid_20Sites = weighted.mean(cattleSlaughter_value_ha_resid/(aux.price.2017), w = pastureArea_value*(muni_site_area/muni_area), na.rm = T),
...
summary(aux.theta.2017)

# add cattleSlaughter_value_ha_fitted and pastureArea_value to spatial variables
calibration.20SitesModel <- left_join(calibration.20SitesModel, aux.theta.2017)

# clean environment
#rm(muniTheta.prepData, aux.theta)

# calculate theta_20Sites
calibration.20SitesModel <-
  calibration.20SitesModel %>%
  dplyr::mutate(theta_scenario2_20Sites = cattleSlaughter_value_ha_scenario2/(aux.price.2017),
                theta_scenario3_20Sites = cattleSlaughter_value_ha_scenario3/(aux.price.2017),
                theta_scenario4_20Sites = cattleSlaughter_value_ha_scenario4/(aux.price.2017),
                theta_scenario5_20Sites = cattleSlaughter_value_ha_scenario5/(aux.price.2017)) %>%
  dplyr::select(-cattleSlaughter_value_ha_scenario2, -cattleSlaughter_value_ha_scenario3,
                -cattleSlaughter_value_ha_scenario4, -cattleSlaughter_value_ha_scenario5)

# identify adjacent neighbors
aux.neighbors <- sf::st_is_within_distance(calibration.20SitesModel, calibration.20SitesModel, dist = 100, remove_self = TRUE)

# impute values for missing thetas based on the average of adjacent neighbors
calibration.20SitesModel <-
  calibration.20SitesModel %>%
  dplyr::mutate(d_imputation_theta_scenario2_20Sites = dplyr::if_else(is.na(theta_scenario2_20Sites), 1, 0),
                theta_scenario2_20Sites = dplyr::if_else(is.na(theta_scenario2_20Sites),
                                                          apply(aux.neighbors, 1, function(i){mean(.$theta_scenario2_20Sites[i], na.rm = TRUE)}),
                                                          theta_scenario2_20Sites),
                d_imputation_theta_scenario3_20Sites = dplyr::if_else(is.na(theta_scenario3_20Sites), 1, 0),
                theta_scenario3_20Sites = dplyr::if_else(is.na(theta_scenario3_20Sites),
                                                          apply(aux.neighbors, 1, function(i){mean(.$theta_scenario3_20Sites[i], na.rm = TRUE)}),
                                                          theta_scenario3_20Sites),
                d_imputation_theta_scenario4_20Sites = dplyr::if_else(is.na(theta_scenario4_20Sites), 1, 0),
                theta_scenario4_20Sites = dplyr::if_else(is.na(theta_scenario4_20Sites),
                                                          apply(aux.neighbors, 1, function(i){mean(.$theta_scenario4_20Sites[i], na.rm = TRUE)}),
                                                          theta_scenario4_20Sites),
                d_imputation_theta_scenario5_20Sites = dplyr::if_else(is.na(theta_scenario5_20Sites), 1, 0),
                theta_scenario5_20Sites = dplyr::if_else(is.na(theta_scenario5_20Sites),
                                                          apply(aux.neighbors, 1, function(i){mean(.$theta_scenario5_20Sites[i], na.rm = TRUE)}),
                                                          theta_scenario5_20Sites))
...
calibration.20SitesModel <-
  calibration.20SitesModel %>%
  dplyr::bind_cols(dplyr::tibble(theta_soybean_20Sites = terra::extract(raster.soybean, terra::vect(calibration.20SitesModel %>% sf::st_transform(4326)), mean, na.rm = T)$soyb200b_yld))
...
# match munis with sites
aux.worker <- sf::st_intersection(calibration.20SitesModel, muniAgCensusCattleRaising.prepData)
...
calibration.20SitesModel <- left_join(calibration.20SitesModel, aux.worker)
...
calibration.20SitesModel <-
  calibration.20SitesModel %>%
  dplyr::mutate(workerPerHa_20Sites = workers_ha_fitted) %>%
...
calibration.20SitesModel <-
  calibration.20SitesModel %>%
  dplyr::mutate(p_2017_20Sites = calibration.globalModel$p_2017_global,
                p_2017_alt_20Sites = calibration.globalModel$p_2017_alt_global,
                pSoybean_2017_20Sites = calibration.globalModel$pSoybean_2017_global,
                pSoybean_2017_alt_20Sites = calibration.globalModel$pSoybean_2017_alt_global)
...
calibration.20SitesModel <-
  calibration.20SitesModel %>%
  dplyr::filter(!is.na(gamma_20Sites))


# ORDER VARIABLES
calibration.20SitesModel <-
  calibration.20SitesModel %>%
  dplyr::select(id, ends_with("ha_20Sites"), cluster, clusterRank,
                theta_20Sites, thetaSD_20Sites, theta_resid_20Sites, theta_residSD_20Sites,
                z_2017_20Sites, zbar_2017_20Sites, x_2017_20Sites, b_20Sites, theta_20Sites, gamma_20Sites, gammaSD_20Sites,
                d_theta_winsorized,
                pastureArea_value, ends_with("_20Sites"))


# POST-TREATMENT OVERVIEW
# summary(calibration.20SitesModel)
# View(calibration.20SitesModel)
...
save(calibration.20SitesModel,
     file = here::here("data/projectSpecific/20SitesModel",
                       "calibration_20SitesModel.Rdata"))

# remove spatial feature
calibration.20SitesModel <- calibration.20SitesModel %>% sf::st_drop_geometry()

readr::write_csv(calibration.20SitesModel,
                 file = here::here("data/projectSpecific/20SitesModel", "calibration_20SitesModel.csv"))
...
ExportTimeProcessing("projectSpecific/20SitesModel")
