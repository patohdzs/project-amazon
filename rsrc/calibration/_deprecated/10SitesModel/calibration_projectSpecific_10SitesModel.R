...
here::i_am("code/projectSpecific/10SitesModel/calibration_projectSpecific_10SitesModel.R", uuid = "0fef2915-0a76-4b7f-9a2c-c0b9830c058b")


# START TIME
tictoc::tic(msg = "calibration_projectSpecific_10SitesModel script", log = T)
...
aux.cluster <- kmeans(aux.clusterData, centers = 10, nstart = 25)
...
ggplot2::ggsave(filename = here::here("data/analysis/10SitesModel/cluster_gammaTheta_1000Sites.png"), width = 10, height = 6)
...
ggplot2::ggsave(filename = here::here("data/analysis/10SitesModel/map_10clusters_1000Sites.png"), width = 8, height = 6)
...
ggplot2::ggsave(filename = here::here("data/analysis/10SitesModel/map_10clustersRankGamma_1000Sites.png"), width = 8, height = 6)

# intermediate export before aggregating
save(calibration.1000SitesModel,
     file = here::here("data/projectSpecific/1000SitesModel",
                       "cluster10Sites_1000SitesModel.Rdata"))
...
calibration.10SitesModel <-
  ...
  dplyr::ungroup() %>%
  sf::st_as_sf() %>%
  dplyr::rename_with(.fn = ~str_replace(., "1000Sites", "10Sites"), .cols = ends_with("1000Sites"))


ggplot2::ggplot(data = calibration.10SitesModel) +
  ggplot2::geom_sf(aes(fill = as.factor(clusterRank))) +
  ggplot2::scale_fill_viridis_d(name = "Cluster") +
  ggplot2::guides(fill = guide_legend(label.position = "bottom", title.position = "top", nrow = 1)) +
  ggplot2::theme(panel.grid.major = element_line(colour = "White"),
...
#   sf::st_transform(sf::st_crs(calibration.10SitesModel)) %>%
#   dplyr::mutate(co2e_ha = (agb/2)*(44/12)) %>%
#   dplyr::select(co2e_ha)
#
# # match minicells with sites
# aux.gamma <- sf::st_join(calibration.10SitesModel, pixelBiomass.prepData)
#
...
#   sf::st_drop_geometry() %>%
#   dplyr::group_by(id) %>%
#   summarise(gamma_10Sites = mean(co2e_ha, na.rm = T))
#
#
# # add gamma_10Sites to spatial variables
# calibration.10SitesModel <- left_join(calibration.10SitesModel, aux.gamma)
#
# # clean environment
...
# calibration.10SitesModel <-
#   calibration.10SitesModel %>%
#   dplyr::mutate(gamma_alt_10Sites = gamma_10Sites*1.101)
...
pixelBiomass2017.prepData %>%
  ...
  fixest::etable(se = "hetero", replace = T, file = here::here("data/analysis/10SitesModel/reg_gamma.tex"), style.tex = fixest::style.tex(main = "qje"))
...
# match minicells with sites
aux.gamma2017 <- sf::st_join(calibration.10SitesModel, pixelBiomass2017.prepData)

# calculate average carbon density on primary forest areas by site
aux.gamma2017 <- aux.gamma2017 %>% sf::st_drop_geometry() %>% dplyr::group_by(id) %>% summarise(gamma_10Sites = mean(co2e_ha_2017, na.rm = T),
                                                                                                co2e_ha_2017 = mean(co2e_ha_2017, na.rm = T),
                                                                                                gammaSD_10Sites = mean(co2eSD_ha_2017_fitted, na.rm = T),
                                                                                                co2eSD_ha_2017 = mean(co2eSD_ha_2017, na.rm = T),
                                                                                                lon = mean(lon, na.rm = T),
                                                                                                lat = mean(lat, na.rm = T))

# add gamma_10Sites to spatial variables
calibration.10SitesModel <- left_join(calibration.10SitesModel, aux.gamma2017)
...
calibration.10SitesModel <-
  calibration.10SitesModel %>%
  dplyr::mutate(gammaSD_10Sites = predict(reg.carbonUncertainty, .),
                gammaSD_10Sites = exp(gammaSD_10Sites))
...
calibration.10SitesModel <-
  calibration.10SitesModel %>%
  dplyr::mutate(a_10Sites = calibration.globalModel$a_global,
                b_10Sites = a_10Sites*gamma_10Sites*zbar_2017_10Sites)
...
calibration.10SitesModel <-
  calibration.10SitesModel %>%
  dplyr::mutate(k_10Sites = calibration.globalModel$k_global)
...
calibration.10SitesModel <-
  calibration.10SitesModel %>%
  dplyr::mutate(zeta_10Sites = calibration.globalModel$zeta_global,
                zeta_alt_10Sites = calibration.globalModel$zeta_alt_global)
...
calibration.10SitesModel <-
  calibration.10SitesModel %>%
  dplyr::mutate(rho_10Sites = calibration.globalModel$rho_global)
...
calibration.10SitesModel <-
  calibration.10SitesModel %>%
  dplyr::mutate(r_10Sites = calibration.globalModel$r_global)
...
calibration.10SitesModel <-
  calibration.10SitesModel %>%
  dplyr::mutate(x_2017_10Sites = gamma_10Sites*forestArea_2017_ha_10Sites)
...
calibration.10SitesModel <-
  calibration.10SitesModel %>%
  dplyr::mutate(C_2017_10Sites = calibration.globalModel$C_2017_global,
                C0bern_2017_10Sites = calibration.globalModel$C0bern_2017_global,
                C1bern_2017_10Sites = calibration.globalModel$C1bern_2017_global,
                C2bern_2017_10Sites = calibration.globalModel$C2bern_2017_global,
                C3bern_2017_10Sites = calibration.globalModel$C3bern_2017_global)
...
calibration.10SitesModel <-
  calibration.10SitesModel %>%
  dplyr::mutate(u_10Sites = calibration.globalModel$u_global,
                u_bern_10Sites = calibration.globalModel$u_bern_global)
...
# change crs to match with calibration.10SitesModel and select variables
muniTheta.prepData <-
  muniTheta.prepData %>%
  sf::st_transform(crs = sf::st_crs(calibration.10SitesModel)) %>%
...
aux.theta <- sf::st_intersection(calibration.10SitesModel, muniTheta.prepData)
...
aux.theta %>%
  ...
  readr::write_csv(here::here("data/projectSpecific/10SitesModel", "10site_muni.csv"))
...
aux.theta.2017 <-
  ...
  sf::st_drop_geometry() %>%
  dplyr::group_by(id) %>%
  dplyr::summarise(theta_10Sites = weighted.mean(cattleSlaughter_value_ha_fitted/(aux.price.2017), w = pastureArea_value*(muni_site_area/muni_area), na.rm = T),
                   thetaSD_10Sites = sqrt(Hmisc::wtd.var(cattleSlaughter_value_ha_fitted/(aux.price.2017), w = pastureArea_value*(muni_site_area/muni_area), na.rm = T)),
                   theta_residSD_10Sites = sqrt(Hmisc::wtd.var(cattleSlaughter_value_ha_resid/(aux.price.2017), w = pastureArea_value*(muni_site_area/muni_area), na.rm = T)),
                   theta_resid_10Sites = weighted.mean(cattleSlaughter_value_ha_resid/(aux.price.2017), w = pastureArea_value*(muni_site_area/muni_area), na.rm = T),
...
calibration.10SitesModel <- left_join(calibration.10SitesModel, aux.theta.2017)

# clean environment
#rm(muniTheta.prepData, aux.theta)

# calculate theta_10Sites
calibration.10SitesModel <-
  calibration.10SitesModel %>%
  dplyr::mutate(theta_scenario2_10Sites = cattleSlaughter_value_ha_scenario2/(aux.price.2017),
                theta_scenario3_10Sites = cattleSlaughter_value_ha_scenario3/(aux.price.2017),
                theta_scenario4_10Sites = cattleSlaughter_value_ha_scenario4/(aux.price.2017),
                theta_scenario5_10Sites = cattleSlaughter_value_ha_scenario5/(aux.price.2017)) %>%
...
calibration.10SitesModel <-
  calibration.10SitesModel %>%
  dplyr::mutate(d_imputation_theta_scenario2_10Sites = dplyr::if_else(is.na(theta_scenario2_10Sites), 1, 0),
                theta_scenario2_10Sites = dplyr::if_else(is.na(theta_scenario2_10Sites),
                                                          apply(aux.neighbors, 1, function(i){mean(.$theta_scenario2_10Sites[i], na.rm = TRUE)}),
                                                          theta_scenario2_10Sites),
                d_imputation_theta_scenario3_10Sites = dplyr::if_else(is.na(theta_scenario3_10Sites), 1, 0),
                theta_scenario3_10Sites = dplyr::if_else(is.na(theta_scenario3_10Sites),
                                                          apply(aux.neighbors, 1, function(i){mean(.$theta_scenario3_10Sites[i], na.rm = TRUE)}),
                                                          theta_scenario3_10Sites),
                d_imputation_theta_scenario4_10Sites = dplyr::if_else(is.na(theta_scenario4_10Sites), 1, 0),
                theta_scenario4_10Sites = dplyr::if_else(is.na(theta_scenario4_10Sites),
                                                          apply(aux.neighbors, 1, function(i){mean(.$theta_scenario4_10Sites[i], na.rm = TRUE)}),
                                                          theta_scenario4_10Sites),
                d_imputation_theta_scenario5_10Sites = dplyr::if_else(is.na(theta_scenario5_10Sites), 1, 0),
                theta_scenario5_10Sites = dplyr::if_else(is.na(theta_scenario5_10Sites),
                                                          apply(aux.neighbors, 1, function(i){mean(.$theta_scenario5_10Sites[i], na.rm = TRUE)}),
                                                          theta_scenario5_10Sites))
...
calibration.10SitesModel <-
  calibration.10SitesModel %>%
  dplyr::bind_cols(dplyr::tibble(theta_soybean_10Sites = terra::extract(raster.soybean, terra::vect(calibration.10SitesModel %>% sf::st_transform(4326)), mean, na.rm = T)$soyb200b_yld))
...
# match munis with sites
aux.worker <- sf::st_intersection(calibration.10SitesModel, muniAgCensusCattleRaising.prepData)
...
calibration.10SitesModel <- left_join(calibration.10SitesModel, aux.worker)
...
calibration.10SitesModel <-
  calibration.10SitesModel %>%
  dplyr::mutate(workerPerHa_10Sites = workers_ha_fitted) %>%
...
calibration.10SitesModel <-
  calibration.10SitesModel %>%
  dplyr::mutate(p_2017_10Sites = calibration.globalModel$p_2017_global,
                p_2017_alt_10Sites = calibration.globalModel$p_2017_alt_global,
                pSoybean_2017_10Sites = calibration.globalModel$pSoybean_2017_global,
                pSoybean_2017_alt_10Sites = calibration.globalModel$pSoybean_2017_alt_global)
...
calibration.10SitesModel <-
  calibration.10SitesModel %>%
  dplyr::filter(!is.na(gamma_10Sites))


# ORDER VARIABLES
calibration.10SitesModel <-
  calibration.10SitesModel %>%
  dplyr::select(id, ends_with("ha_10Sites"), cluster, clusterRank,
                theta_10Sites, thetaSD_10Sites, theta_resid_10Sites, theta_residSD_10Sites,
                z_2017_10Sites, zbar_2017_10Sites, x_2017_10Sites, b_10Sites, theta_10Sites, gamma_10Sites, gammaSD_10Sites,
                d_theta_winsorized,
                pastureArea_value, ends_with("_10Sites"))
...
save(calibration.10SitesModel,
     file = here::here("data/projectSpecific/10SitesModel",
                       "calibration_10SitesModel.Rdata"))
...
readr::write_csv(calibration.10SitesModel,
                 file = here::here("data/projectSpecific/10SitesModel", "calibration_10SitesModel.csv"))
...
ExportTimeProcessing("projectSpecific/10SitesModel")
