...
here::i_am("code/projectSpecific/prepData/pixelBiomass2020_projectSpecific_prepData.R", uuid = "19c04f37-96dc-4e8d-a2ae-ade294205749")


# START TIME
tictoc::tic(msg = "pixelBiomass2020_projectSpecific_prepData script", log = T)
...
agb.raster <- purrr::map(list.files(here::here("data/raw2clean/abovegroundBiomass_kayrros/input/2020"), pattern = ".tif", full.names = T),
                         terra::rast)
...
# remove duplicates
pixelBiomass2020.prepData <- dplyr::distinct(pixelBiomass2020.prepData, id, .keep_all = TRUE)

# check if all points were extracted
if (length(pixelBiomass2020.prepData$id) != length(pixelPrimaryForest.prepData$id)) {
...
# LABELS
sjlabelled::set_label(pixelBiomass2020.prepData$agb_2020) <- "aboveground biomass in 2020 (Mg per ha)"



# POST-TREATMENT OVERVIEW
# summary(pixelBiomass2020.prepData)
# View(pixelBiomass2020.prepData)
...
save(pixelBiomass2020.prepData,
     file = here::here("data/projectSpecific/prepData",
                       paste0("pixelBiomass2020_prepData", ".Rdata")))
