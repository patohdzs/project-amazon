...
raster.10Sites <- terra::rast(list.files(paste(getwd(), "data/calibration/1043SitesModel/aux_tifs", sep = "/"),
                                         pattern = "raster_",
                                         full.names = T))
...
site.gamma2017 <-
  sf::st_intersection(calibration.10SitesModel %>% dplyr::select(id),
