...
site.gamma2017 <-
  sf::st_intersection(calibration.1043SitesModel %>% dplyr::select(id),
...
aux.gamma2017 <-
  site.gamma2017 %>%
  dplyr::group_by(id) %>%
  dplyr::summarise(gamma2017_1043Sites = weighted.mean(co2e_ha_2017_fitted, w=muni_site_area, na.rm = T))
...
id <-calibration.1043SitesModel %>%
  select(id)

st_write(id, "data/hmc/id_1043.geojson", driver = "GeoJSON",delete_dsn = TRUE)
...
readr::write_csv(calibration.1043SitesModel,
                 file = here::here("data/hmc", "hmc_1043SitesModel.csv"))
