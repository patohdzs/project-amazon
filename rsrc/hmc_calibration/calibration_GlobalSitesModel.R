...
pixelBiomass2017.prepData <-
  pixelBiomass2017.prepData %>%
  sf::st_transform(sf::st_crs(calibration.1000SitesModel)) %>%
...
calibration.globalModel <-
  calibration.globalModel %>%
  dplyr::bind_cols(dplyr::tibble(gamma2017_global = mean(pixelBiomass2017.prepData$co2e_ha_2017)))
...
readr::write_csv(calibration.globalModel,
                 file = paste(getwd(), "data/calibration/globalModel", "calibration_globalModel.csv", sep = "/"))
