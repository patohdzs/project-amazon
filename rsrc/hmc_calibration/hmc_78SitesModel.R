...
raster.78Sites$forestArea_2008_ha_78Sites <-
  raster.78Sites$share_forest_2008 * raster.78Sites$pixelArea_ha
raster.78Sites$agriculturalUseArea_2008_ha_78Sites <-
  raster.78Sites$share_agriculturalUse_2008 * raster.78Sites$pixelArea_ha
raster.78Sites$otherArea_2008_ha_78Sites <-
  raster.78Sites$share_other_2008 * raster.78Sites$pixelArea_ha

# select area variables
# select area variables
raster.78Sites <- terra::subset(
  raster.78Sites,
  c(
    ...
    "forestArea_2008_ha_78Sites",
    "agriculturalUseArea_2008_ha_78Sites",
    "otherArea_2008_ha_78Sites"
  )
)
...
calibration.78SitesModel <-
  ...
  dplyr::select(
    ...
    forestArea_2008_ha_78Sites,
    otherArea_2008_ha_78Sites,
    z_2008_78Sites = agriculturalUseArea_2008_ha_78Sites,
    zbar_2008_78Sites
  )
...
reg.gamma.2017 <-
  muniTheta.prepData  %>%
  lm(
    formula = log(co2e_ha_2017)  ~ log(historical_precip) + log(historical_temp) +
      log(lat) + log(lon),
...
muniTheta.prepData  <-   muniTheta.prepData %>%
  mutate(co2e_ha_2017_fitted = exp(predict(reg.gamma.2017, .) + 0.5 *
                                     sigma(reg.gamma.2017) ^ 2))
...
site.gamma2017 <-
  sf::st_intersection(
    calibration.78SitesModel %>% dplyr::select(id),
...
aux.gamma2017 <-
  site.gamma2017 %>%
  dplyr::group_by(id) %>%
  dplyr::summarise(gamma2017_78Sites = weighted.mean(co2e_ha_2017_fitted, w =
                                                       muni_site_area, na.rm = T))


# add gamma_78Sites to spatial variables
calibration.78SitesModel <-
  dplyr::left_join(calibration.78SitesModel, aux.gamma2017)
...
calibration.78SitesModel <-
  calibration.78SitesModel %>%
  dplyr::mutate(gamma2017_78Sites = dplyr::if_else(
    is.na(gamma2017_78Sites),
...
calibration.78SitesModel <-
  calibration.78SitesModel %>%
  dplyr::group_by(id) %>%
  dplyr::mutate(gamma_78Sites = rowMeans(across(c(
    "gamma2017_78Sites"
  )))) %>%
...
zeta_alt <-
  483 / aux.transitionArea # Alternative value based on a quote from (https://www.otempo.com.br/brasil/investigacoes-revelam-quadrilhas-e-ganho-milionario-por-tras-do-desmate-1.2229571)
...
muniTheta.prepData_filtered <- muniTheta.prepData %>%
  ...
  mutate(weights_pasture_area_2017 = pasture_area_2017 / mean(pasture_area_2017)) # renormalize pasture area
...
reg.cattleValueperHa.2017 <-
  ...
  lm(
    formula = log(cattleSlaughter_valuePerHa_2017) ~ historical_precip + historical_temp +
      I(historical_temp ^ 2) + lat + I(lat ^ 2) + distance + cattleSlaughter_farmGatePrice_2017,
    na.action = na.exclude,
    weights = weights_pasture_area_2017
  )
...
muniTheta.prepData <-
  muniTheta.prepData %>%
  mutate(cattleSlaughter_valuePerHa_fitted_2017 = exp(
    predict(reg.cattleValueperHa.2017, .) + 0.5 * sigma(reg.cattleValueperHa.2017) ^ 2
  ))
...
muniTheta.prepData <-
  muniTheta.prepData %>%
  dplyr::mutate(
    d_theta_winsorized_2017 = dplyr::if_else(cattleSlaughter_valuePerHa_fitted_2017 <= 0,
                                             1,
                                             0)
  )
# clean environment
...
aux.theta.2017 <-
  ...
  dplyr::group_by(id) %>%
  dplyr::summarise(
    theta2017_78Sites = weighted.mean(
      cattleSlaughter_valuePerHa_fitted_2017 / aux.price.2017,
      w = muni_site_area,
      na.rm = T
    ),
    pasture_area_2017 = sum(pasture_area_2017 * (muni_site_area /
                                                   muni_area), na.rm = T),
...
calibration.78SitesModel <-
  dplyr::left_join(calibration.78SitesModel, aux.theta.2017)
...
seriesPriceCattle.prepData <-
  seriesPriceCattle.prepData %>%
  dplyr::mutate(
    price_low = quantile(price_real_mon_cattle, 0.25),
    # define low price value as the 33th percentile
    price_high = quantile(price_real_mon_cattle, 0.75),
    # define high price value as the 66th percentile
...
seriesPriceCattle.prepData$d_high <-
  as.numeric(NA) # initialize dummy indicating if the price is high or low
seriesPriceCattle.prepData[1, "d_high"] <-
  1 # initial value set to 1 because the first price is the highest of the series

for (i in 2:nrow(seriesPriceCattle.prepData)) {
  # a change from high to low only occurs if price reaches a value below the low
  if (seriesPriceCattle.prepData[i - 1, "d_high"] == 1 &
      seriesPriceCattle.prepData[i, "price_real_mon_cattle"] > seriesPriceCattle.prepData[i, "price_low"]) {
    seriesPriceCattle.prepData[i, "d_high"] <- 1
  } else if (seriesPriceCattle.prepData[i - 1, "d_high"] == 1 &
             seriesPriceCattle.prepData[i, "price_real_mon_cattle"] < seriesPriceCattle.prepData[i, "price_low"]) {
    seriesPriceCattle.prepData[i, "d_high"] <- 0
    # a change from low to high only occurs if price reaches a value above the high
  } else if (seriesPriceCattle.prepData[i - 1, "d_high"] == 0 &
             seriesPriceCattle.prepData[i, "price_real_mon_cattle"] > seriesPriceCattle.prepData[i, "price_high"]) {
    seriesPriceCattle.prepData[i, "d_high"] <- 1
  } else if (seriesPriceCattle.prepData[i - 1, "d_high"] == 0 &
             seriesPriceCattle.prepData[i, "price_real_mon_cattle"] < seriesPriceCattle.prepData[i, "price_high"]) {
    seriesPriceCattle.prepData[i, "d_high"] <- 0
...
matrixTransition.2prices <-
  markovchain::markovchainFit(seriesPriceCattle.prepData$discrete_2prices)$estimate@transitionMatrix
...
calibration.78SitesModel <-
  calibration.78SitesModel %>% sf::st_drop_geometry()
