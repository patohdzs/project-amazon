    ...
    def __init__(self, configs={}):
        ...
        if not isinstance(configs, dict):
            ...
            if self.validate_configurations(configs):
                ...
                print(
                    "The passed configurations dictionary includes invlid values/types"
                )
                raise TypeError
...
class HMCSampler(Sampler):
    ...
    _DEF_CONFIGURATIONS = {
        ...
        # 'mass_matrix':np.ones(13),
        "mass_matrix": np.eye(13),
    }
    ...
    def _update_mass_matrix(self):
        ...
        if sparse is not None:
            ...
            print("mass matrix used in HMC:", mass_matrix)
        else:
            ...
            def func_grad(x, fd_eps=fd_eps, fd_central=fd_central):
                ...
                for i in range(e.size):
                    ...
                    if fd_central:
                        ...
                        grad[i] = (
                            func(x + e) - func(x)
                        ) / fd_eps  ## need to reconsider here
                return grad
    ...
    def _update_log_density(self):
        ...
        if not callable(log_density):
            print(
                "The 'log_density' found in the configurations is not a valid callable/function!"
            )
            raise TypeError
        try:
        ...
        if log_density_grad is None:
            ...
            def log_density_grad_parallel(x):
                return self.__parallel_func_grad(
                    x, processes=min(size, multiprocess.cpu_count())
                )

            def log_density_grad_threaded(x):
                return self.__threaded_func_grad(
                    x, processes=min(size, multiprocess.cpu_count())
                )
            ...
            print(
                "The 'log_density_grad' found in the configurations is not a valid callable/function!"
            )
            raise TypeError
    ...
    def validate_configurations(self, configs, raise_for_invalid=True):
        ...
        if not callable(log_density):
            if raise_for_invalid:
                print("The 'log_density' is not a valid callable/function")
                raise TypeError
            else:
                is_valid = False
                return is_valid
    ...
    def apply_symplectic_integration(
        ...
        if step_size <= 0:
            print("Step size of the symplectic integrator must be positive!!")
            raise ValueError
        ...
        for _ in range(num_steps):
            ...
            if re.match(r"\A(verlet|leapfrog)\Z", symplectic_integrator, re.IGNORECASE):
                if np.iscomplexobj(current_state):
                    print("complex state.", current_state)
                ...
                if np.iscomplexobj(proposed_state):
                    print("complex state.", proposed_state)
                    print("inverse mass matrix", self._MASS_MATRIX_INV)
