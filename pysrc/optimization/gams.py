...
def solve_planner_problem(
    ...
    if model == "prediction":
        pa_data = pd.DataFrame({"price": [pa] * 200})
        ...
        pa_data = pd.DataFrame({"price": prices})
        pa_data.index = range(1, 201)
        pa_data.index.name = None
        saveto = os.path.join(working_directory, "p_a.csv")
        pa_data.to_csv(saveto, header=True, index_label=None)
    ...
    ws = GamsWorkspace(
        system_directory=_DATA_DIR + "/gams45.1_linux_x64_64_sfx",
        working_directory=_DATA_DIR + f"/{num_sites}sites/",
    )
...
def read_dat_file(filename):
    ...
    # Temporarily store the current Y value and the list of numbers.
    current_y = None
    values = []
    p_a = None
