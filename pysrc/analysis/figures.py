...
def land_allocation(pee=7.6,num_sites=1043,opt='gams',pa=41.11,model='det'):
    ...
    custom_labels = ["$p^{{ee}}$={0}       $b$".format(pee), "0", "10", "15", "20", "25"]
    ...
    plt.legend(
        loc="upper center", bbox_to_anchor=(0.5, -0.15), ncol=5, frameon=False, fontsize=16
    )
    plt.savefig(
    ...
    plt.xlim(0, max(time) + 2)
    plt.legend(
        loc="upper center", bbox_to_anchor=(0.5, -0.15), ncol=5, frameon=False, fontsize=18
    )
    plt.savefig(
...
def density(pee=7.6,num_sites=78,opt='gams',pa=41.11,xi=1,model='det'):
    ...
    result_folder = os.path.join(str(get_path("output")), "sampling",model,opt,f'{num_sites}sites',f'pa_{pa}' ,f'xi_{xi}')
    prior_folder = os.path.join(str(get_path("output")), "sampling","prior",f'{num_sites}sites')
    ...
    for idx in range(num_sites):
        ...
        global_min = min(gamma_unadjusted[:, idx].min(), gamma_adjusted_b0[:, idx].min(),gamma_adjusted_b15[:, idx].min())
        global_max = max(gamma_unadjusted[:, idx].max(), gamma_adjusted_b0[:, idx].max(),gamma_adjusted_b15[:, idx].max())
        bin_edges = np.linspace(global_min, global_max, num_bins+1)
        
        sns.kdeplot(gamma_unadjusted[:, idx], label='baseline', color='black', fill=False, alpha=0.6,linewidth=4)  # Bright blue
        sns.kdeplot(gamma_adjusted_b0[:, idx], label='b=0', color='red', fill=True, alpha=0.6,linewidth=4)  # Bright red
        sns.kdeplot(gamma_adjusted_b15[:, idx], label='b=15', color='blue', fill=True, alpha=0.6,linewidth=4)  # Bright red

        plt.title(f'Probability density for $\gamma$ and site {idx+1}',fontsize=16)
    ...
    for idx in range(num_sites):
        ...
        global_min = min(theta_unadjusted[:, idx].min(), theta_adjusted_b0[:, idx].min(),theta_adjusted_b15[:, idx].min())
        global_max = max(theta_unadjusted[:, idx].max(), theta_adjusted_b0[:, idx].max(),theta_adjusted_b15[:, idx].max())
        bin_edges = np.linspace(global_min, global_max, num_bins+1)
        

        sns.kdeplot(theta_unadjusted[:, idx], label='baseline', color='black', fill=False, alpha=0.6,linewidth=4) # Bright blue
        sns.kdeplot(theta_adjusted_b0[:, idx], label='b=0', color='red', fill=True, alpha=0.6,linewidth=4)  # Bright red
        sns.kdeplot(theta_adjusted_b15[:, idx], label='b=15', color='blue', fill=True, alpha=0.6,linewidth=4)  # Bright red
        
        plt.title(f'Probability density for $\Theta$ and site {idx+1}',fontsize=16)
        plt.xlabel('parameter value',fontsize=16)
        plt.ylabel('density',fontsize=16)
        plt.legend(fontsize=16)
...
def trajectory_diff(num_sites=78,pe_hmc=7.1,pe_det=5.3,b=0,opt="gams",pa=41.11):
    ...
    dfz_hmc= pd.read_csv(result_folder+'/hmc/'+opt+f"/78sites/pa_{pa}/"+f"pe_{pe_hmc}/amazon_data_z.dat", delimiter='\t')
    ...
    for i in range(51):
        result_zper_hmc[i] =(np.sum(dfz_zeronp_hmc[i])/(np.sum(dfz_bar_np)/1e9))*100
        
        
    dfz_det= pd.read_csv(result_folder+'/det/'+opt+f"/78sites/pa_{pa}/"+f"pe_{pe_det}/amazon_data_z.dat", delimiter='\t')
    ...
    for i in range(51):
        result_zper_det[i] =(np.sum(dfz_zeronp_det[i])/(np.sum(dfz_bar_np)/1e9))*100
    ...
    plt.savefig(output_folder+f'/aggregate_percentage_Z_b{b}_pehmc_{pe_hmc}_pedet_{pe_det}.png')
    plt.show()
