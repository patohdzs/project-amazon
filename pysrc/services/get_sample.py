...
def get_sampling(opt="gams",
    ...
    for pe in pe_values:
        ...
        output_base_path=os.path.join(str(get_path("output")), "sampling",opt,f'{num_sites}sites',f'pa_{pa}' ,f'xi_{xi}',f'pe_{pe}')
...
def get_prior(num_sites=78):
    ...
    results_dir = os.path.join(str(get_path("output")), "sampling","prior",f'{num_sites}sites')
    if not os.path.exists(results_dir):
        os.makedirs(results_dir)

    filename = f"prior.pcl"
