...
def get_optimization(opt="gams",
    ...
    if model == "det":
        ...
        for pe in pe_values:
            ...
            subfolder_path = os.path.join(output_base_path, "optimization",model,opt,f'{num_sites}sites',f'pa_{pa}' ,f'pe_{pe}')
    ...
    if model == "hmc":    
        result_folder = os.path.join(str(get_path("output")), "sampling",opt,f'{num_sites}sites',f'pa_{pa}' ,f'xi_{xi}')
        b = [0, 10, 15, 20, 25]
        pe_values = [pee + bi for bi in b]
        for pe in pe_values:
